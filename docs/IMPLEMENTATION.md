...
> **HISTORICAL DOCUMENT (cleanup-phase-1).** This was the original
> integration plan for adding slmodemd to synthmodem. The plan
> centered on a host-paced backend with a small in-VM helper called
> `modemd-shim` that bridged audio + control between Node and
> slmodemd over wire-framed TCP. Section 4.1 documents that helper.
> **That backend was removed in cleanup-phase-1.** The current
> shipping architecture is described in `README.md` and the
> implementation reference is `slmodemd-pjsip.md`. This document is
> kept for context — milestone history, the rationale behind certain
> early choices, and the cross-platform / licensing analysis that
> still applies — but is **not authoritative for current code**.
>
> Specifically:
> - Section 4.1 (`modemd-shim`) describes a deleted helper.
> - Section 4.3 (host-side Node components) names
>   `SlmodemBackend.js`, deleted in cleanup-phase-1.
> - Section 5 (wire protocol) describes the host-paced wire format;
>   the current control-plane wire format is in `vm/ctrl/wire.h`.
> - Section 8 (build and release flow) describes a single-binary
>   build flow; the current two-script workflow is in
>   `MAINTAINERS.md`.
> - Section 10 (milestones) is a development log; current state
>   should be read from `README.md` and `Handoff.md` Section 0.
>
> Read `README.md`, `QUICKSTART.md`, `MAINTAINERS.md`,
> `slmodemd-pjsip.md`, and `PJSIP.md` for current, authoritative
> content.
...
- macOS host support
- ARM host support
- ~~Automatic backend selection ("auto" mode)~~ — **superseded
  2026-04-30**: the `auto` backend now ships, composing
  slmodemd-pjsip and native via deterministic V.8-timeout
  discrimination. See `CallSession.js` (`_dspIsPooled`/
  `_everConnected` swap trigger), `RtpSession.adoptSocket`, and
  the auto-mode coverage in `README.md`/`QUICKSTART.md`/`Handoff.md`.
