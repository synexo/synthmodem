...
> **DESIGN DOCUMENT.** This is the *why* — the motivation, high-level
> architecture, and rationale — for what is now the default
> `slmodemd-pjsip` backend (cleanup-phase-1 removed the older
> host-paced backend, so what this document calls "Option B" is now
> the production path). The references to "Option A" / "Option B" in
> this document are historical: Option A described an alternative
> approach (host-paced PJSIP) which was never implemented. The
> "Option B" architecture described here is what shipped.
>
> For the *implementation manual* — how the pieces are laid out, what
> talks to what, how to build them, how to debug them — see
> `slmodemd-pjsip.md`. For the user-facing description, see
> `README.md`.
>
> Specifically still accurate in this document:
> - The B2BUA topology
> - The tunnel design
> - The role of d-modem.c
> - The risk register
>
> No longer accurate:
> - Build script names (the workflow is now wrapped by
>   `scripts/vendor-sources.sh` and `scripts/rebuild-prebuilts.sh`
>   per `MAINTAINERS.md`)
> - References to a "shim ↔ slmodemd audio interface" that this
>   document positions itself relative to — that's the host-paced
>   backend, removed.
