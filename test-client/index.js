 ...
 *  4. Once the modem handshake completes, provides interactive terminal I/O
 *     that mirrors what a real modem user would experience
 *
 * Note: as of cleanup-phase-2 the test client is best-effort. It was
 * intended as a no-PSTN integration smoke test for the native backend
 * but has bit-rotted through several backend refactors. Phase 5 owns
 * its revival. Speaker playback (--audio / --volume) was removed in
 * Phase 2 along with the `speaker` npm dependency.
 *
 * Usage:
 *   node test-client/index.js [options]
 *
 * Options:
 *   --proto PROTO   Force protocol (V21|V22|Bell103|V22bis|V23)
 *                   V22bis and V23 are TESTING — opt-in only
...
Options:
  --proto  <name>   Force protocol (advertise via V.8 CM):
                      V21  V22  Bell103     (active, default-advertised)
                      V22bis  V23           (TESTING — opt-in only)
...
if (getArg('--proto',  null)) {
  ...
  config.modem.native.advertiseProtocol = getArg('--proto', null);
}
...
async function main() {
  ...
  info(`Protocol:   ${config.modem.native.advertiseProtocol || config.modem.native.forceProtocol || 'auto (V.8 negotiation)'}`);
  console.log('');

  // Local speaker playback removed in cleanup-phase-2 (the `speaker`
  // dependency was the test client's last reason to need a native
  // build toolchain on install). The test client now does SIP/RTP
  // and the originating-side modem only.
