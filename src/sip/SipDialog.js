...
class SipDialog extends EventEmitter {
  ...
  constructor({ callId, localTag, remoteTag, localUri, remoteUri, transport, remoteAddr, remotePort }) {
    ...
    // publicHost resolved for THIS dialog (set by SipServer at INVITE).
    // 200 OK and BYE both read from here so the IP advertised in
    // Contact / SDP / Via is consistent across the whole dialog.
    // Source/iface are kept for diagnostic logging.
    this.localPublicHost       = null;
    this.localPublicHostSource = null;
    this.localPublicHostIface  = null;
