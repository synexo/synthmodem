...
class SipServer extends EventEmitter {
  ...
  _handleInvite(msg, addr, port, transport, socket) {
    ...
    const ringing = parser.buildResponse(msg, 180, 'Ringing', {
      ...
      Contact: `<sip:synthmodem@${cfg.publicHost}:${cfg.port}>`,
    });
  ...
  _build200Ok(request, dlg) {
    ...
    return parser.buildResponse(request, 200, 'OK', {
      To:             toHeader,
      Contact:        `<sip:synthmodem@${cfg.publicHost}:${cfg.port};transport=${dlg.transport}>`,
  ...
  sendBye(dlg) {
    ...
    bye.setHeader('Via',        `SIP/2.0/${dlg.transport.toUpperCase()} ${cfg.publicHost}:${cfg.port};branch=${parser.generateBranch()}`);
