 ...
 *
 * ── cleanup-phase-2 retention note (April 2026) ─────────────────────────
 *
 * The following classes are NOT used by any currently-active protocol
 * (V.21, V.22, Bell 103) after the cleanup-phase-2 removal of spandsp,
 * V.32bis, and V.34:
 *
 *   AGC            — was used by ModemDSP.js for inbound level
 *                    normalisation; disabled by default and removed in
 *                    Phase 2 because no surviving protocol benefited
 *   CostasLoop     — carrier recovery loop (was used by V.34's pure-JS
 *                    QAM demodulator)
 *   GardnerTiming  — symbol-timing recovery (was used by V.34)
 *   LMSEqualizer   — adaptive equaliser (was used by V.34 and the old
 *                    pre-spandsp V.22bis)
 *   Scrambler      — V.34-style self-synchronising scrambler (was used
 *                    by V.32bis and V.34; the V.22 PUREJS implementation
 *                    has its own internal V22Scrambler class and does
 *                    NOT use this one)
 *
 * They are intentionally retained here as building blocks / reference
 * material for any future native-V.22bis fix work. The pure-JS V.22bis
 * class preserved in V22.js is TESTING-status and not known-working;
 * fixing it (or implementing a fresh native V.22bis) will likely lean
 * on these primitives rather than reinvent them. Removing them now
 * would force that future work to either re-derive each one from a
 * spec or recover them from git history — not worth the small win in
 * surface-area trim.
 *
 * If a future cleanup phase concludes that V.22bis is being deleted
 * outright (rather than fixed), these classes lose their last
 * justification and can be removed at that time.
 * ────────────────────────────────────────────────────────────────────────
 */
 ...
 *
 * Disabled by default in config.modem.native.agcEnabled; ModemDSP skips calling
