                                           ...
                                           void *put_bit_user_data)
{
    ...
    /* Upstream v32bis.c calls modem_echo_can_init(256), but that API
     * does not exist in the shipped modem_echo.c (only
     * modem_echo_can_segment_init does).  SynthModem does not need
     * the echo canceller — our SIP/RTP path handles echo upstream,
     * and V.32bis's echo canceller is intended for true 4-wire PSTN
     * operation where the modem must subtract its own TX from the
     * RX mix.  We zero the pointer and patch release/free to match. */
    s->ec = NULL;
...
{
    /* Paired with the NULL init above — nothing to free here. */
    s->ec = NULL;
    return 0;
}
/*- End of function --------------------------------------------------------*/
