...
/**
 * ModemBackendPool — manages a pre-warmed VM-backed modem backend
 * (currently PjsipBackend / slmodemd-pjsip) so calls can activate
 * the modem without paying VM boot cost at call time.
 *
 * ─── Why pre-warming matters ────────────────────────────────────
 *
 * A cold VM takes ~9 seconds to boot (QEMU TCG + Linux kernel +
 * userland init + slmodemd + d-modem + PJSIP REGISTER). During
 * that window:
...
class ModemBackendPool extends EventEmitter {
  /**
   * @param {object} opts
   * @param {object} [opts.backendOpts] — opts passed to each backend
   *     constructor. Contents depend on the factory; for the
   *     PjsipBackend factory used by index.js this is
   *     `{ qemuPath, kernelPath, initrdPath, role, ... }`.
   * @param {(opts:object)=>EventEmitter} opts.backendFactory —
   *     callable that constructs a backend from backendOpts. REQUIRED.
   *
   *     The returned object must expose at minimum:
   *       startAsync(): Promise<void>
   *       stop():       Promise<void> | void
   *     and emit 'error'. Any additional methods (activate, receiveAudio,
   *     write, etc.) are dictated by the caller (usually CallSession).
   *
  ...
  constructor(opts = {}) {
    super();
    if (typeof opts.backendFactory !== 'function') {
      throw new Error(
        'ModemBackendPool: opts.backendFactory is required ' +
        '(must be a function returning an EventEmitter-like backend).');
    }
    this._backendOpts = opts.backendOpts || {};
    this._backendFactory = opts.backendFactory;
    this._warmupTimeoutMs = opts.warmupTimeoutMs ?? 30000;

    /** @type {'empty'|'warming'|'ready'|'checked_out'|'stopped'} */
    this._state = 'empty';

    /** @type {EventEmitter|null} The currently-warm backend, if any. */
    this._backend = null;

    /** @type {Promise<EventEmitter>|null} Warmup in progress. */
   ...
   *
   * @returns {Promise<EventEmitter>} the warmed-up backend (PjsipBackend)
   */
  async checkout() {
    if (this._stopped) {
   ...
   *
   * @param {EventEmitter} backend — the just-used backend
   */
  recycle(backend) {
    if (this._stopped) {
  ...
  async _warmupOne() {
    ...
    this._warmupPromise = (async () => {
      const bk = this._backendFactory(this._backendOpts);
