/*
 * wire.h — synthmodem <-> modemd-shim wire protocol
 *
 ...
 *
 *   - modemd-shim (running inside the guest VM, or in M1 directly on the
 *     host next to slmodemd), which speaks this protocol over two
 *     virtio-serial ports (or, in M1, two Unix sockets), AND
 *
 *   - the Node host-side driver (SlmodemVM.js in vm/qemu-runner/),
 *     which speaks this same protocol over the corresponding host-side
 *     Unix sockets / named pipes.
 *
 * Two logically separate channels exist:
 *
 *   AUDIO CHANNEL   — carries PCM16 audio frames both directions.
 *                     One TCP-like stream; guest and host both read
 *                     and write frames.
 *
 *   CONTROL CHANNEL — carries AT commands (host→guest), AT responses
 *                     and parsed modem status events (guest→host),
 *                     post-CONNECT modem data in both directions,
 *                     and a single startup HELLO from guest→host that
 *                     signals "shim is up and ready".
 *
 * ─────────────────────────────────────────────────────────────────────────
 *
 * Frame format (identical on both channels):
 *
 *     +-------------+---------+--------------------------+
 *     |  length     |  type   |  payload                 |
 ...
 *     (so newer shim/host can roll in new message types without
 ...
 *
 * The shim performs all impedance matching between this protocol and
 * slmodemd's native interfaces (socketpair audio + PTY AT/data).
 *
 * ─────────────────────────────────────────────────────────────────────────
 *
 ...
 *   3. Add encode/decode support in BOTH wire.c (shim) AND
 ...
 *     "modemd-shim v<WIRE_VERSION> build=<build-id>"
 *
 * The shim emits exactly one HELLO as its first message on the control
 * channel, immediately after opening the virtio-serial / socket. The
 * host uses HELLO reception as its "VM/shim is ready" signal and
 * should not send any messages before receiving HELLO.
 *
