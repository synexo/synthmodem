...
# The minimal runtime initramfs we'll extend. If it doesn't exist yet,
# build it (cheap — it's the normal 'make' target).
if [ ! -f "$VM_DIR/images/rootfs.cpio.gz" ]; then
  echo "Building runtime initramfs first (vm/images/rootfs.cpio.gz)…"
  (cd "$VM_DIR" && make --no-print-directory)
fi
...
echo "Base: extracting runtime initramfs…"
(cd "$BUILDER_ROOT" && zcat "$VM_DIR/images/rootfs.cpio.gz" | cpio -i --quiet --make-directories)
...
echo "Adding shim source tree at /src/shim…"
cp -a "$VM_DIR/shim" "$BUILDER_ROOT/src/"
# Scrub any host-built binaries from the source copies — the VM must
# build from scratch to get provenance clean.
rm -f "$BUILDER_ROOT/src/shim/modemd-shim" \
      "$BUILDER_ROOT/src/shim/modemd-shim-i386"
...
echo ""
echo "building modemd-shim..."
cd /src/shim
make clean 2>&1 | tail -1
# Inside the i386 VM, gcc's default is already 32-bit, so the
# "native" target IS the i386 target. We build modemd-shim
# (the native name), then rename to modemd-shim-i386 for clarity
# when it lives in vm/prebuilt/.
make modemd-shim 2>&1 | tail -5

if [ -x modemd-shim ]; then
    echo ""
    echo "=== modemd-shim build OK ==="
    ls -la modemd-shim
    echo "glibc symbols:"
    strings modemd-shim 2>/dev/null | grep -E '^GLIBC_[0-9.]+$' | sort -u | sed 's/^/  /'
    if strings modemd-shim 2>/dev/null | grep -q __isoc23; then
        echo "WARNING: modemd-shim still has __isoc23_ symbols"
    fi
    cp modemd-shim /output/modemd-shim-i386
    SHIM_OK=1
fi

sync
echo ""
if [ $SLMODEMD_OK = 1 ] && [ $SHIM_OK = 1 ]; then
    echo "BUILD_SUCCESS"
else
    echo "BUILD_FAILURE (slmodemd=$SLMODEMD_OK shim=$SHIM_OK)"
fi
...
if [ ! -f "$OUTPUT_DIR/modemd-shim-i386" ]; then
  err "Builder VM did not produce modemd-shim-i386"
fi
...
cp "$OUTPUT_DIR/modemd-shim-i386" "$VM_DIR/prebuilt/modemd-shim-i386"
chmod +x "$VM_DIR/prebuilt/slmodemd" "$VM_DIR/prebuilt/modemd-shim-i386"

SLMODEMD_HASH=$(sha256sum "$VM_DIR/prebuilt/slmodemd"         | awk '{print $1}')
SHIM_HASH=$(    sha256sum "$VM_DIR/prebuilt/modemd-shim-i386" | awk '{print $1}')
echo "  slmodemd            SHA256: $SLMODEMD_HASH"
echo "                      Size:   $(stat -c%s "$VM_DIR/prebuilt/slmodemd") bytes"
echo "  modemd-shim-i386    SHA256: $SHIM_HASH"
echo "                      Size:   $(stat -c%s "$VM_DIR/prebuilt/modemd-shim-i386") bytes"
